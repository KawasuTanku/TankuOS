...
/// `$XDG_CONFIG_HOME/tankuos/systems.toml` (or `~/.config/tankuos/systems.toml`),
...
fn systems_file_path(
    ...
    Some(base.join("TankuOS").join("systems.toml"))
}
...
/// tankuos (mac / linux / WSL2) + gpm (Linux console mouse) on the remote.
...
echo 'tankuos: installing gpm (console mouse)…'; \
sudo apt-get install -y gpm 2>/dev/null || sudo dnf install -y gpm 2>/dev/null || \
sudo pacman -S --noconfirm gpm 2>/dev/null || echo 'tankuos: could not install gpm automatically'; \
command -v systemctl >/dev/null 2>&1 && sudo systemctl enable --now gpm 2>/dev/null; fi; true",
        );
        s.push_str(&format!("echo \"── tankuos: setting up\" {name} \"──\"\n"));
        // 1. Make sure we have a key to transfer.
        s.push_str(
            "mkdir -p \"$HOME/.ssh\"\n\
if [ ! -f \"$HOME/.ssh/id_ed25519\" ] && [ ! -f \"$HOME/.ssh/id_rsa\" ]; then\n\
  echo 'tankuos: generating an SSH key (~/.ssh/id_ed25519)…'\n\
  ssh-keygen -t ed25519 -N '' -f \"$HOME/.ssh/id_ed25519\"\n\
        ...
        s.push_str(&format!(
            "echo \"tankuos: [1/5] copying SSH key to\" {target} \"…\"\n\
if [ -n \"${{SSHPASS:-}}\" ] && command -v sshpass >/dev/null 2>&1; then\n\
  sshpass -e ssh-copy-id {pf}{target} || {{ echo 'tankuos: ERROR — key transfer failed (wrong host/password?)'; exit 1; }}\n\
else\n\
  [ -n \"${{SSHPASS:-}}\" ] && echo 'tankuos: sshpass not installed — enter the password when prompted.'\n\
  ssh-copy-id {pf}{target} || {{ echo 'tankuos: ERROR — key transfer failed (wrong host/password?)'; exit 1; }}\n\
fi\n\
echo 'tankuos: key transfer OK'\n",
        ));
        ...
        s.push_str(&format!(
            "echo \"tankuos: [2/5] teaching the remote your terminal type ($TERM)…\"\n\
if command -v infocmp >/dev/null 2>&1 && infocmp -x \"$TERM\" >/dev/null 2>&1; then\n\
  infocmp -x \"$TERM\" | ssh {pf}{target} 'mkdir -p ~/.terminfo && tic -x -' \\\n\
    && echo 'tankuos: terminfo installed' \\\n\
    || echo 'tankuos: terminfo copy failed — remote will fall back to xterm-256color'\n\
else\n\
  echo 'tankuos: no local terminfo for '\"$TERM\"' — remote will fall back to xterm-256color'\n\
fi\n",
        ));
        // 4. Install tankuos + gpm on the remote.
        s.push_str(&format!(
            "echo 'tankuos: [3/5] installing tankuos on the remote (mac/linux/wsl2)…'\n\
ssh -t {pf}{target} {install} || {{ echo 'tankuos: ERROR — remote install failed (output above)'; exit 1; }}\n\
echo 'tankuos: [4/5] checking gpm (Linux console mouse)…'\n\
ssh -t {pf}{target} {gpm}\n",
        ));
        // 5. Sync the saved-systems list so the assistant (and the Systems
        //    menu) on the remote knows the same machines as here.
        s.push_str(&format!(
            "echo 'tankuos: [5/5] syncing your saved systems — connecting…'\n\
if [ -f \"$HOME/.config/tankuos/systems.toml\" ]; then\n\
  ssh {pf}{target} 'mkdir -p ~/.config/tankuos' \\\n\
    && scp {spf}\"$HOME/.config/tankuos/systems.toml\" {target}:.config/tankuos/ >/dev/null \\\n\
    || echo 'tankuos: systems sync failed (non-fatal)'\n\
fi\n",
        ));
    }
...
/// (keeping a `.tankuos.bak`), and leaves any other keys untouched. A missing
...
in=\"$ak.tankuos.in\"; out=\"$ak.tankuos.tmp\"; \
cat > \"$in\" || { rm -f \"$in\"; exit 2; }; \
cp \"$ak\" \"$ak.tankuos.bak\" || { echo 'tankuos: backup failed'; rm -f \"$in\"; exit 2; }; \
...
| grep -v '^$' | sort -u > \"$kf\"; \
if [ ! -s \"$kf\" ]; then echo 'tankuos: no local SSH public key to revoke'; rm -f \"$kf\"; exit 0; fi; \
...
mod tests {
    ...
    fn plain_switch_script_is_one_exec_ssh() {
        ...
        assert!(s.contains("TANKUOS_THEME=") && s.contains("nord"), "theme rides over ssh: {s}");
    ...
    fn revoke_script_filters_our_key_best_effort() {
        ...
        assert!(s.contains(".tankuos.bak"), "keeps a backup: {s}");
        ...
        assert!(s.contains("$ak.tankuos.tmp"), "writes the result inside ~/.ssh, not /tmp: {s}");
    ...
    fn setup_script_has_key_install_gpm_then_attach() {
        ...
        let install = s.find("install.sh").expect("installs tankuos remotely");
        ...
        assert!(!s.contains("TANKUOS_THEME"), "no theme env when the system has none");
    }
}
