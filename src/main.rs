//! tankuos entry point — a thin dispatcher over the daemon/client split.
//!
...
fn main() -> std::io::Result<()> {
    ...
    match cmd.as_deref() {
        ...
        Some(other) => {
            eprintln!(
                "tankuos: unknown command '{other}' (try: attach, kill, kill-app, ps, reload, launch, tile, theme, msg, service, --daemon)"
            );
            Ok(())
        }
        None => attach(true),
...
fn ctl(msg: &tankuos::session::ClientMsg) -> std::io::Result<()> {
    if send_control(msg)? {
        ...
        eprintln!("tankuos: no daemon running (start it with `tankuos`)");
    }
    Ok(())
}
...
fn attach(spawn_if_missing: bool) -> std::io::Result<()> {
    loop {
        ...
        if UnixStream::connect(&path).is_err() {
            ...
            if !ready {
                eprintln!("tankuos: daemon failed to start");
                return Ok(());
            }
        }
...
fn run_switch(spec: &tankuos::systems::SwitchSpec) {
    ...
    // Give the user a beat to read any setup/ssh output before tankuos's
    // alternate screen swallows it on re-attach.
    if spec.setup {
        println!("tankuos: re-attaching to the local session in 3s… (Ctrl-C to stay in the shell)");
...
/// "2d04h"). Used by `tankuos ps` and the in-app activity monitor.
...
fn fetch_app_list() -> std::io::Result<Vec<tankuos::apphost::AppListEntry>> {
    ...
    let proto: u32 = match tankuos::apphost::proto::recv::<HostEvt, _>(&mut r)? {
        ...
        None => {
            eprintln!("tankuos: apphost closed before sending roster");
            std::process::exit(1);
        }
    };
    if proto < LIST_APPS_MIN_PROTO && proto != 0 {
        ...
        eprintln!(
            "tankuos: apphost speaks protocol v{proto}; 'ps' / 'kill-app' require v{LIST_APPS_MIN_PROTO}+ \
    ...
    eprintln!(
        "tankuos: apphost did not reply with an AppList after {MAX_DRAIN} events \
